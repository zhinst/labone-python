...
class Session:
    ...
    TESTED_CAPABILITY_VERSION = version.Version("1.15.0")
    ...
    def ensure_compatibility(self) -> None:
        ...
        if self._capability_version.major > Session.TESTED_CAPABILITY_VERSION.major:
            msg = (
                "The data server version is incompatible with this LabOne API "
                "version. Please install the latest python package and retry. "
                f"({self._capability_version}> {Session.TESTED_CAPABILITY_VERSION})"
            )
            raise errors.UnavailableError(msg)
